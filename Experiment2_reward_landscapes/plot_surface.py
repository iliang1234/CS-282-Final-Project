"""
    ...
    NOTE: code citation: Hao et. al, 2018, Loss Landscape research
"""
import argparse
import copy
import h5py
...
def setup_surface_file(args, surf_file, dir_file):
    ...
    if os.path.exists(surf_file):
        print("direction file already exists, ", surf_file)
        print("----")
    ...
    print("ranges: ", args.xmin, args.xmax, args.xnum)
...
def crunch(surf_file, net, w, s, d, dataloader, loss_key, acc_key, comm, rank, args):
    ...
    losses, accuracies = [], []
    print("crunching: ", f['xcoordinates'])
    print("----")
    print(f['xcoordinates'])
    print("----")
    ...
    for count, ind in enumerate(inds):
        ...
        if rank == 0:
            # TODO: write losses and accuracies (100 by 100)
            # pdb.set_trace()
            f[loss_key][:] = losses
            f[acc_key][:] = accuracies
            # clears internal buffer, does not affect file content
            f.flush()
    ...
    print("SHAPE: ", np.array(losses).shape,  np.array(accuracies).shape)
    f.close()
...
if __name__ == '__main__':
    ...
    try:
        ...
        if args.y:
            print("we've reached y argument here")
    ...
    #--------------------------------------------------------------------------
    # loads the dataset and model together
    ...
    #--------------------------------------------------------------------------
    # unclear what direction file and surface file are
    dir_file = net_plotter.name_direction_file(args) # name the direction file

    print("borders: ", args.xmin, args.xmax)
    if rank == 0:
    ...
    if args.plot and rank == 0:
        assert args.y
        assert not args.proj_file
        print("in this case")
        individual_plot.plot_d_contour(surf_file, 'train_loss', args.vmin, args.vmax, args.vlevel, args.show)
